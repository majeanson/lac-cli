 ...
 */
export function generateHelpWidget(
  _features: Feature[],
  projectName: string,
  options: { guideUrl?: string } = {},
): string {
  const guideUrl = options.guideUrl ?? './lac-guide.html'

  // The JS is returned as a string literal — single quotes inside are escaped.
  return `/*!
